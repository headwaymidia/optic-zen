...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: lead, error: leadErr } = await admin.from("leads").select("id, name, phone, lead_source, store_id, created_at").eq("id", lead_id).maybeSingle();
    if (leadErr) throw leadErr;
    if (!lead) throw new Error("Lead não encontrado");
    if (lead.lead_source !== "WhatsApp") {
      return new Response(JSON.stringify({
        skipped: true,
        reason: "lead_source != WhatsApp"
      }), {
        headers: {
          ...corsHeaders,
    ...
    const { data: store } = await admin.from("stores").select("id, name, owner_id").eq("id", lead.store_id).maybeSingle();
    if (!store?.owner_id) throw new Error("Loja sem dono");
    const { data: ownerProfile } = await admin.from("profiles").select("email, full_name").eq("id", store.owner_id).maybeSingle();
  ...
  <div style="max-width:560px;margin:0 auto;background:#fff;border-radius:12px;padding:32px">
    ...
    <div style="text-align:center;margin:32px 0">
      <a href="https://www.oticadominante.com.br" style="background:#111;color:#fff;text-decoration:none;padding:14px 28px;border-radius:8px;display:inline-block;font-weight:600">Atender agora</a>
    </div>
